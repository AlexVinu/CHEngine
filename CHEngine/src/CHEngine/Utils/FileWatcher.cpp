...
void FileWatcher::Watch(const std::filesystem::path& path, Callback callback)
{
    if (!std::filesystem::exists(path)) {
        CHE_CORE_WARN("FileWatcher: file not found, watch was not started: {0}", path.string().c_str());
        return;
    }
    ...
    CHE_CORE_TRACE("FileWatcher: watching '{0}'", path.filename().string().c_str());
}
...
void FileWatcher::Poll()
{
    for (auto& entry : m_Entries) {
        ...
        if (currentWrite != entry.LastWrite) {
            ...
            CHE_CORE_INFO("FileWatcher: '{0}' changed - reloading...", entry.Path.filename().string().c_str());
            if (entry.OnChanged)
